...
if torch.cuda.is_available():
    ...
    device = 'cpu'
def train(pretraining:bool, documentModel:str,queryModel:str,save_path:str,batchsz:int=64,accum_steps:int =4,save_steps:int=3000,global_steps:int=0,warmup_steps:int = 400,lambda_q:int = 0.01,lambda_d:int = 0.008,epochs:int = 5,use_untie_model:bool = False):
    ...
    if use_untie_model:
        ...
        '''
        untie the embeddings matrix and initialize the matrix with the embeddings matrix
        '''
        modelDocument.init_output_embeddings(modelDocument.get_output_embeddings())
        modelQuery.init_output_embeddings(modelQuery.get_output_embeddings())
    else:
        ...
        for batch in (pbar:=tqdm.tqdm(train_dataloader,desc=f"epoch {epoch}")):
            ...
            with torch.autocast(device_type=device):
                ...
                if not pretraining:
                    ...
                    loss = mseloss(margin_pred,labels)
