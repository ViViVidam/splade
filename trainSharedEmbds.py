...
def train(documentModel:str,queryModel:str,save_path:str,batchsz:int=64,accum_steps:int =4,save_steps:int=3000,global_steps:int=0,warmup_steps:int = 400,lambda_q:int = 0.01,lambda_d:int = 0.008,epochs:int = 5,use_untie_model:bool = False):
    ...
    modelDocument = SpladeFusionsharedEmbds.from_pretrained(documentModel).to(device)
    modelQuery = SpladeFusionsharedEmbds.from_pretrained(queryModel).to(device)
    modelDocument.decoder.load_state_dict(torch.load(""))
    modelDocument.cls.load_state_dict(torch.load("/expanse/lustre/projects/csb176/yzound/checkpoint/splade/msmarco-bert-co-condensor-cls/model_state_dict.pt",weights_only=True))
    modelQuery.decoder.load_state_dict(torch.load(""))
    modelQuery.cls.load_state_dict(torch.load("/expanse/lustre/projects/csb176/yzound/checkpoint/splade/msmarco-bert-co-condensor-cls/model_state_dict.pt",weights_only=True))
    ...
    for epoch in range(epochs):
        for batch in (pbar:=tqdm.tqdm(train_dataloader,desc=f"epoch {epoch}")):
            ...
            pbar.set_postfix_str(f"accumulated loss: {loss_accum/steps}, loss: {loss.item()}")
